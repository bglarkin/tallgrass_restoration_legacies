...
[^2]: The switchgrass fields at Fermilab could not be included because no plant data exists for these fields.
...
```{r dbRDA-all,fig.width=4.5,fig.height=3.5,fig.cap="Results of a distance-based redundancy analysis of soil fungal community composition in restored fields from all regions. Icons represent the constrained locations of fields. Solid blue arrows show the direction and strength of associations between communities and explanatory variables. Dotted blue arrows show correlations with strong, but non-significant explanatory variables. Axis 1 is significant based on 1999 permutations at p=0.008."}
...
Soil fungal communities in cornfields are very different from those in restored or prairie remnant fields. That's not surprising, and neither is the rapid change in communities as the years pass after agricultural management ceases and restoration starts. With this project, we had hoped to find evidence that restored fields either become more like remnants over time, or that they become something new entirely. There are examples of both in the literature, particularly with bacterial communities. What we found instead is generally weak effects and inconclusive results. Communities in restored fields do change and years since restoration is the strongest correlating variable with change, but it's hard to say whether they become more like remnants or not. With soil fungi, it looks like communities in restorations trend away from remnant communities as they age (Fig \@ref(fig:its-samps-pcoa), but restored and remnant communities appear indistinguishable for AMF (Fig \@ref(fig:amf-samps-pcoa)). With so few replicates, these differences are even harder to detect. 

Richness and diversity indices tend to align restored fields with remnants for both soil fungi and AMF (Figs \@ref(fig:its-div) and \@ref(fig:amf-div)). What this suggests more than anything else is that cornfields are a difficult place for fungi to live, filtering communities for those species that can tolerate extreme environments. The discovery of numerous extremophile taxa as indicators of cornfields supports this interpretation. 

Looking at soil fungi in guilds (Fig \@ref(fig:guilds-regions)), cornfields contain more OTUs assigned to nearly every guild, and little summary exists to separate restored from remnant fields. But drawing conclusions based on these data may be fraught anyway. The agricultural industry certainly knows that the soil microbiome matters for production, if for no other reason than monitoring pathogens. I'm concerned that this added attention inflates metadata from agricultural species in trait and guild databases. Since 54% of OTUs were unassigned at the genus level, it's entirely possible that the trends we've observed in guilds among field types could be reversed. With AMF in families (table \@ref(tab:family-abund)), the situation is similar in that differences among field types are weak, but at least with AMF, we avoid the unknown bias of trait databases. 
...
The fact that years since restoration is the strongest explanatory variable for community change in restored fields is compelling because it suggests that the rules for community change after restoration, whatever they are, might generalize across regions. The problem I have with these results is that our chronosequence is compromised. I went back and looked at ordinations based on plant communities and found that years since restoration is also a strongly correlated explanatory variable (Fig \@ref(fig:plant-yr)), also collinear with forbs and C4 grasses. If this change in plant communities were natural, it'd be fine, but we know from landowners and restoration managers that the older restoration practice was to plant mostly grass, and now, forbs are almost exclusively planted. So is the change we see in soil microbial communities due to some natural process over time or simply to the choice of plant species? I think it's both because C4 grasses do tend to increase in restored fields over time on their own, but we don't have any way to separate these confounded explanations. 


***

Soil fungal communities in cornfields differ significantly from those in restored or prairie remnant fields, showing a noticeable shift over the years following the cessation of agricultural management. Our project aimed to determine whether restored fields evolve towards remnant characteristics or develop unique profiles. Contrary to expectations, the results indicate generally weak effects and inconclusive outcomes. While communities in restored fields undergo changes, especially over time since restoration, determining if they converge with remnant communities remains challenging.

Analysis of soil fungi reveals a trend of restored fields moving away from remnant communities as they age (Fig @ref(fig:its-samps-pcoa)), but no such distinction is observed for arbuscular mycorrhizal fungi (AMF) (Fig @ref(fig:amf-samps-pcoa)). The limited number of replicates makes these differences harder to discern.

Richness and diversity indices align restored fields with remnants for both soil fungi and AMF (Figs @ref(fig:its-div) and @ref(fig:amf-div)), suggesting that cornfields exert selective pressure on fungi, favoring species resilient to extreme conditions. The identification of extremophile taxa as indicators of cornfields supports this interpretation.

Examining soil fungi in guilds reveals that cornfields host more Operational Taxonomic Units (OTUs) across various guilds, making it challenging to differentiate restored from remnant fields. However, drawing conclusions from these data may be precarious, given potential inflation of metadata from agricultural species in trait and guild databases.

While the influence of years since restoration emerges as a significant variable in community change in restored fields, concerns arise about the compromised nature of our chronosequence. Analysis of plant communities also correlates strongly with years since restoration, raising questions about whether observed changes in soil microbial communities are due to natural processes or the choice of plant species.

In summary, our findings emphasize the complexity of soil microbial dynamics in restored fields, with implications for understanding community changes and the need for further research to disentangle confounding factors.
...
Finally, after seeing the glorious restorations at Fermilab, I think we expected similar results in Wisconsin. Restored fields in Wisconsin had nice plant communities, but were often weedy and the soil seemed beat up and hard in most cases. Teh restoration practice was to stop plowing and sow native seeds. That's it. Reintroduction of fire isn't practical in these populated rural areas, and no other soil amendments (e.g., to restore soil carbon) were attempted as far as I could find out. 

We also expected the remnants to provide compelling reference conditions. Instead, remnants were often on slopes where or within railroad rights-of-way where they escaped the plow, but had been grazed and neglected, small fragments with substantial cover of exotics or woodland invasion due to lack of fire. The Faville Grove remnant was the only example of a true reference condition, in my opinion. 

I would love to hear your ideas about what I could be missing here. 